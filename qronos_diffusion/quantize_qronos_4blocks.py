...
from qronos_w4a4 import QronosW4A4
...
QUANT_BITS = 4
GROUP_SIZE = 128
CHECKPOINT_DIR = "./qronos_4block_checkpoints"
...
def check_integrity(fpath):
    """Try loading the file to ensure it's not corrupt."""
    try:
        # Just map to CPU to verify structure
...
def find_latest_checkpoint():
    ...
    # Identify all possible step indices
    steps = []
    ...
    # Sort descending to find the latest one first
    steps.sort(reverse=True)
    
    for step in steps:
        model_path = os.path.join(CHECKPOINT_DIR, f"model_block_{step}.pt")
        inputs_path = os.path.join(CHECKPOINT_DIR, f"inputs_block_{step}.pt")
        
        # Verify both files exist
        if os.path.exists(inputs_path):
            print(f"Checking integrity of checkpoint block {step}...")
            # Verify the INPUTS file specifically (often the largest and most prone to corruption)
            if check_integrity(inputs_path):
                print(f"  > Integrity OK.")
                return model_path, step
            else:
                print(f"  > CORRUPTED. Deleting and skipping block {step}.")
                try: 
                    if os.path.exists(model_path): os.remove(model_path)
                    if os.path.exists(inputs_path): os.remove(inputs_path)
                except: pass
...
def main():
    ...
    if latest_ckpt:
        print(f"\n[RESUME] Resuming from valid checkpoint: {latest_ckpt}")
    ...
    if cached_inputs is None:
        ...
        with torch.no_grad():
            for i, prompt in enumerate(CALIB_PROMPTS):
                print(f"Processing Prompt {i+1}/{len(CALIB_PROMPTS)}", end="\r")
                pipe(prompt, num_inference_steps=CALIB_STEPS)
        
        hook.remove()
        cached_inputs = capturer.inputs
        
        # Save baseline inputs (block -1)
    ...
    # --- PHASE 2: SEQUENTIAL BLOCK QUANTIZATION ---
    print(f"\n--- Phase 2: Sequential Quantization (Blocks {start_block} to {BLOCK_LIMIT-1}) ---")
    ...
    for i in range(start_block, BLOCK_LIMIT):
        ...
        handlers = {name: QronosW4A4(layer) for name, layer in layers}
        
        for h in handlers.values():
            h.dev = torch.device(DEVICE)
            h.H = h.H.to(DEVICE)
            h.G = h.G.to(DEVICE)
        ...
        # 2. Quantize
        print(f"  Quantizing {len(handlers)} layers (Adaptive Dampening)...")
        for layer_idx, (name, layer) in enumerate(layers):
            ...
            handler.fasterquant(qparams, blocksize=128, groupsize=GROUP_SIZE, beta=1e4, percdamp=damp)
            handler.H = None; handler.G = None
        print("")
            
        del handlers
        gc.collect(); torch.cuda.empty_cache()
        
        # 3. Update Inputs (Error Propagation)
    ...
    final_path = os.path.join(CHECKPOINT_DIR, "pixart_4block_qronos.pt")
    torch.save(model.state_dict(), final_path)
    print(f"\n[DONE] 4-Block Qronos Model saved to: {final_path}")
